    ...
    function get_settings_sections() {
        $sections = array(
            ...
            array(
                ...
                'title' => __( '常规设置', 'master-slider' )
            )
        );

        $sections[] = array(
            'id' => 'msp_advanced',
            'title' => __( '高级设置', 'master-slider' )
        );

        $sections[] = array(
            'id' => 'upgrade_to_pro',
            'title' => __( '升级至 Pro 版本', 'master-slider' )
        );
    ...
    function get_settings_fields() {
        ...
        $settings_fields['msp_general_setting'] = array(
            array(
                ...
                'label' => __( '隐藏信息表', 'master-slider' ),
                'desc'  => __( '如果要隐藏Master Slider管理面板上的“最新视频教程”表，请选中此项。', 'master-slider' ),
                'type'  => 'checkbox'
            ),
            array(
                'name'  => '_enable_cache',
                'label' => __( '启用缓存?', 'master-slider' ),
                'desc'  => __( '启用缓存以使Master Slider更快！', 'master-slider' ),
                'type'  => 'checkbox'
            ),
            array(
                'name'  => '_cache_period',
                'label' => __( '缓存周期时间', 'master-slider' ),
                'desc'  => __( '缓存刷新时间（小时）。当您单击滑块面板中的“保存更改”时，缓存也会被清除。', 'master-slider' ),
                'type'  => 'text',
                'default' => '12',
        ...
        $settings_fields['msp_advanced'] = array(
            array(
                ...
                'label' => __( '在所有页面上加载资源？', 'master-slider' ),
                'desc'  => __( '默认情况下，Master Slider将按需加载相应的javascript文件。但如果需要在所有页面上加载资产，请选中此选项。（例如，如果计划通过Ajax加载主滑块，则需要选中此选项）', 'master-slider' ),
                'type'  => 'checkbox'
            )
        );
        ...
        if( 1 == get_option( 'master-slider_ab_pro_feature_setting_content_type', 2 ) ){
            ...
            $settings_fields['upgrade_to_pro'] = array(
                array(
                    'name' => 'upgrade_text',
                    'desc' => __( '升级到pro版本以解锁更多功能！', 'master-slider' ) . sprintf( ' <a href="http://avt.li/mslset" target="_blank">%s</a>', __( 'Checkout the list of features ..', 'master-slider' ) ),
                    'type' => 'plain_text',
                    'label'=> __( '需要更多功能？', 'master-slider' )
                )
            );
