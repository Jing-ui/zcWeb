...
function wp_pagenavi( $args = array() ) {
	...
	$class_names = array(
		...
		'previouspostslink' => apply_filters( 'wp_pagenavi_class_previouspostslink', 'prev' ),
		'extend' => apply_filters( 'wp_pagenavi_class_extend', 'dots' ),
		...
		'nextpostslink' => apply_filters( 'wp_pagenavi_class_nextpostslink', 'next'),
	...
	switch ( intval( $options['style'] ) ) {
		...
		case 1:
			...
			if ( false && $start_page >= 2 && $pages_to_show < $total_pages ) {
				// First
        ...
        $out .= '<div class="nav-links">';
				$out .= $instance->get_single( $paged - 1, $options['prev_text'], array(
					'class' => "{$class_names['previouspostslink']} {$class_names['page']}",
					'rel'   => 'prev'
        ) );
        $out .= "\r\n";
			}
			...
			if ( $start_page >= 2 && $pages_to_show < $total_pages ) {
				if ( !empty( $options['dotleft_text'] ) )
					$out .= "<span class='{$class_names['page']} {$class_names['extend']}'>{$options['dotleft_text']}</span>";
			}
          ...
          $out .= "<span aria-current='page' class='{$class_names['page']} {$class_names['current']}'>$current_page_text</span>";
          $out .= "\r\n";
					$timeline = 'larger';
				} else {
          ...
          $out .= "\r\n";
			...
			if ( $larger_page_out ) {
				$out .= "<span class='{$class_names['page']} {$class_names['extend']}'>{$options['dotright_text']}</span>";
			}
			$out .= $larger_page_out;
			...
			if ( $paged < $total_pages && !empty( $options['next_text'] ) ) {
				$out .= $instance->get_single( $paged + 1, $options['next_text'], array(
					'class' => "{$class_names['nextpostslink']} {$class_names['page']}",
					'rel'   => 'next'
        ) );
        $out .= "\r\n";
        $out .= '</div>';
